...
function [output]=KF_Iter_M_WB_new(ns,P,it,fs,estlpc)
% if nargin < 4
%     it = 3;
% end
...
% The first frame is assumed to be the unvoiced frame. However, I suggest to 
% use a VAD to estimated and update the additive noise variance.
...
for n=1:size(y,2)
   ...
   Q = estimatevariance(y(:,n),w_len,fliplr(-A(n,2:end)),P);
   q= mean(Q);
   if q<0
       q = 1e-8;
   end
%     waitbar(n/size(y,2),hwb,['Please wait... ' num2str(fix(100*n/size(y,2))) ' %'])
    ...
    for kk = 1:it
        ...
        for i=start:w_len
            ...
            P_=F*S*F'+H'*H*q; %Use This equation from now;
